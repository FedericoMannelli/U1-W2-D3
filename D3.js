...
const femaleCharacters = [{name: "Leia Organa", hair_color: "brown", eye_color: "brown"}]
console.log();
...
for (let i = 0; i < eyeColor.length; i++) {
  const element = eyeColor[i];
  
}

switch (eye_Color) {
  case 'blue':
    eyeColor.blue.push('Luke Skywalker'.Luke, "", Skywalker);
    break;
  case 'yellow':
    eyeColor.yellow.push('C-3PO'.C3PO);
    break;
  case 'red':
    eyeColor.red.push();
    break;
  case 'blue-gray':
    eyeColor.blue-gray.push();
    break;
  case 'brown':
    eyeColor.brown.push();
    break;
